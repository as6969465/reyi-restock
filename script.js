...
function renderDeskDefectItems() {
  ...
  const subReasons = `<div class="grid gap-1 mt-2" style="grid-template-columns:repeat(4,1fr)">${DEFECT_REASONS.map(r=>{const s=(item.reasons||[]).includes(r);return `<button type="button" onclick="deskToggleSubReason(${i},'${r}')" style="padding:4px 2px;border-radius:6px;border:1.5px solid ${s?'#2563eb':'#e5e7eb'};background:${s?'#dbeafe':'#f8fafc'};color:${s?'#1d4ed8':'#6b7280'};font-size:10px;font-weight:${s?'700':'400'};cursor:pointer;line-height:1.3;text-align:center;word-break:break-all">${r}</button>`;}).join('')}</div>`;
...
function renderReviewPhotoPanel(p) {
  ...
  // 原因勾選（預填 + 可修改）
  const subReasons = `<div class="grid gap-1 mt-2" style="grid-template-columns:repeat(4,1fr)">
    ${DEFECT_REASONS.map(r=>{
      const s=(cur.reasons||[]).includes(r);
      return `<button type="button" onclick="deskRvToggleReason(${i},'${r}')"
        style="padding:4px 2px;border-radius:6px;border:1.5px solid ${s?'#2563eb':'#e5e7eb'};
          background:${s?'#dbeafe':'#f8fafc'};color:${s?'#1d4ed8':'#6b7280'};
          font-size:10px;font-weight:${s?'700':'400'};cursor:pointer;line-height:1.3;
