...
function importManualExcel(input) {
  ...
  reader.onload = async e => {
    try {
      ...
      if (hRow < 0) { showToast('找不到欄位標題，請確認格式','error',4000); input.value=''; return; }
      const headers = rows[hRow].map(String);
      const idx = {seq:headers.findIndex(h=>h==='序'),po:headers.findIndex(h=>h.includes('採購單號')),cat:headers.findIndex(h=>h.includes('大分類')),barcode:headers.findIndex(h=>h.includes('條碼')),itemNo:headers.findIndex(h=>h==='品號'),name:headers.findIndex(h=>h==='品名'),spec:headers.findIndex(h=>h.includes('規格')),period:headers.findIndex(h=>h.includes('期數')),qty:headers.findIndex(h=>h.includes('採購數量')),price:headers.findIndex(h=>h.includes('售價')),arrival:headers.findIndex(h=>h.includes('到貨日'))};
      const parsed = [];
      for (let i = hRow+1; i < rows.length; i++) {
      ...
      const doImport = async () => {
        try {
          const result = await ProductAPI.importItems(parsed, importDate, {isManual:true});
          showToast(`臨時到貨匯入成功　共 ${result?.inserted||0} 筆新增`, 'success', 4000);
        } catch(e) { showToast('雲端同步失敗：'+e.message,'error',4000); }
        await reloadFromFirestore(importDate);
        closeManualAddModal(); renderProductTable(); updateStats();
      };
      // 重複防呆
...
function renderDeskDefectItems() {
  ...
  const photoThumbs = photos.map((ph, pi) => `
    <div style="position:relative;flex-shrink:0">
      <img src="${ph.src}" onclick="deskViewDefectEntryPhoto(${i},${pi})"
        style="width:72px;height:72px;border-radius:10px;object-fit:cover;cursor:pointer;display:block;border:1.5px solid #93c5fd" />
  ...
  const addPhotoBtn = `
    <label style="width:72px;height:72px;border:2px dashed #93c5fd;border-radius:10px;background:#eff6ff;display:inline-flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;gap:3px;flex-shrink:0">
      ${camSvg}<span style="font-size:10px;color:#93c5fd">新增</span>
      <input type="file" accept="image/*" multiple class="hidden" onchange="deskAddDefectEntryPhotos(${i},this)" />
    </label>`;

  const qtyEl = `<input type="number" min="0" value="${item.qty||''}" placeholder="異常數量"
    class="desk-defect-qty-input"
  ...
  container.innerHTML = `
    ...
    <div style="background:#eff6ff;border-radius:12px;border:1.5px solid #bfdbfe;padding:12px;position:relative">
      <button onclick="deskRemoveDefectItem(${i})" style="position:absolute;top:8px;right:10px;background:none;border:none;color:#93c5fd;cursor:pointer;font-size:12px;padding:2px 4px">✕ 刪除</button>
