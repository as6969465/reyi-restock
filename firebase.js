...
const ProductAPI={
  ...
  async importItems(items,date){let n=0;for(const p of items){const ad=p.arrivalDate||date;const docId=(ad+'_'+(p.itemNo||'x')+'_'+(p.po||'x')).replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,100);const ref=db.collection(COL.products).doc(docId);const snap=await ref.get();if(!snap.exists){await ref.set({arrival_date:ad,seq:p.seq||0,po:p.po||'',cat:p.cat||'',barcode:p.barcode||'',item_no:p.itemNo||'',name:p.name||'',spec:p.spec||'',period:p.period||'',qty:p.qty||0,selling_price:p.sellingPrice||0,is_manual:0,status:'pending',received:false,good_qty:0,bad_qty:0,defect_time:'',defect_class:'其他異常',defect_reasons:[],defect_note:'',defect_staff:'',proc_action:'',proc_reply:'',proc_reply_time:'',proc_staff_name:'',operator_id:'',operator_name:'',photos:[],recv_time:''});n++;}}return{inserted:n};},
