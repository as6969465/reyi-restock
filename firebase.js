...
const ProductAPI={
  ...
  async reply(id,data){const u=getCurrentUser();const s=await db.collection(COL.products).doc(id).get();let dt=s.exists?(s.data().defect_time||''):'';if(dt.endsWith('～'))dt+=new Date().toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false});const _ri={proc_action:data.procAction||'',proc_reply:data.procReply||'',proc_staff_name:u?.name||'',proc_reply_time:nowStr(),defect_time:dt,status:'resolved',proc_reply_unread:true};if(data.defectItems)_ri.defect_items=data.defectItems;await db.collection(COL.products).doc(id).update(_ri);return{status:'resolved'};}
};
