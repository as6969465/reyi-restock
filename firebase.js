 ...
 * 優先使用 config.js 的設定，fallback 到內建設定
 * （Firebase Web API Key 屬公開資訊，安全控制靠 Firestore Security Rules）
 */
...
const ProductAPI={
  ...
  async importItems(items,date,opts){let n=0;const isManual=opts?.isManual?1:0;for(const p of items){const ad=p.arrivalDate||date;const pfx=isManual?'manual_':'';const docId=(pfx+ad+'_'+(p.itemNo||'x')+'_'+(p.po||'x')).replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,100);const ref=db.collection(COL.products).doc(docId);const snap=await ref.get();if(!snap.exists){await ref.set({arrival_date:ad,seq:p.seq||0,po:p.po||'',cat:p.cat||'',barcode:p.barcode||'',item_no:p.itemNo||'',name:p.name||'',spec:p.spec||'',period:p.period||'',qty:p.qty||0,selling_price:p.sellingPrice||0,is_manual:isManual,status:'pending',received:false,good_qty:0,bad_qty:0,defect_time:'',defect_class:'其他異常',defect_reasons:[],defect_note:'',defect_staff:'',proc_action:'',proc_reply:'',proc_reply_time:'',proc_staff_name:'',operator_id:'',operator_name:'',photos:[],recv_time:''});n++;}}return{inserted:n};},
  async create(data){const ref=await db.collection(COL.products).add({arrival_date:data.arrivalDate||new Date().toLocaleDateString('sv-SE'),seq:data.seq||0,po:data.po||'',cat:data.cat||'',barcode:data.barcode||'',item_no:data.itemNo||'',name:data.name,spec:data.spec||'',period:'',qty:data.qty||0,selling_price:data.sellingPrice||0,is_manual:1,status:'pending',received:false,good_qty:0,bad_qty:0,defect_time:'',defect_class:'其他異常',defect_reasons:[],defect_note:'',defect_staff:'',proc_action:'',proc_reply:'',proc_reply_time:'',proc_staff_name:'',operator_id:'',operator_name:'',photos:[],recv_time:''});return{id:ref.id};},
  async setArrived(id,value){const u=getCurrentUser();await db.collection(COL.products).doc(id).update({is_arrived:value,arrived_time:value?nowStr():'',arrived_by:value?u?.name||'':''});},
  ...
  async reply(id,data){const u=getCurrentUser();const s=await db.collection(COL.products).doc(id).get();let dt=data.defectTime||(s.exists?(s.data().defect_time||''):'');if(dt.endsWith('～'))dt+=new Date().toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false});const _ri={proc_action:data.procAction||'',proc_reply:data.procReply||'',proc_staff_name:u?.name||'',proc_reply_time:nowStr(),defect_time:dt,status:data.status||'resolved',proc_reply_unread:true};if(data.defectItems)_ri.defect_items=data.defectItems;await db.collection(COL.products).doc(id).update(_ri);return{status:'resolved'};}
};

// ── 業務屬性 API ──────────────────────────────────────
const BizAttrAPI = {
  async list() {
    const snap = await db.collection('biz_attrs').orderBy('created_at').get();
    return snap.docs.map(d=>({id:d.id,...d.data()}));
  },
  async create(name) {
    if (!name.trim()) throw new Error('請輸入屬性名稱');
    const ref = await db.collection('biz_attrs').add({name:name.trim(),createdAt:nowStr(),created_at:firebase.firestore.FieldValue.serverTimestamp()});
    return {id:ref.id,name:name.trim()};
  },
  async delete(id) {
    await db.collection('biz_attrs').doc(id).delete();
  }
};
