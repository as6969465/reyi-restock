...
const ProductAPI={
  async getByDate(date){return(await db.collection(COL.products).where('arrival_date','==',date).orderBy('seq').get()).docs.map(d=>({id:d.id,...d.data()}));},
  ...
  async importItems(items,date){let n=0;for(const p of items){const ad=p.arrivalDate||date;const ex=await db.collection(COL.products).where('arrival_date','==',ad).where('item_no','==',p.itemNo||'').where('po','==',p.po||'').get();if(ex.empty){await db.collection(COL.products).add({arrival_date:ad,seq:p.seq||0,po:p.po||'',cat:p.cat||'',barcode:p.barcode||'',item_no:p.itemNo||'',name:p.name||'',spec:p.spec||'',period:p.period||'',qty:p.qty||0,is_manual:0,status:'pending',received:false,good_qty:0,bad_qty:0,defect_time:'',defect_class:'其他異常',defect_reasons:[],defect_note:'',defect_staff:'',proc_action:'',proc_reply:'',proc_reply_time:'',proc_staff_name:'',operator_id:'',operator_name:'',photos:[],recv_time:''});n++;}}return{inserted:n};},
  ...
  async receive(id,data){const u=getCurrentUser();const st=(data.badQty||0)>0?'abnormal_pending':'received';await db.collection(COL.products).doc(id).update({received:true,good_qty:data.goodQty||0,bad_qty:data.badQty||0,defect_class:data.defectClass||'其他異常',defect_reasons:data.defectReasons||[],defect_note:data.defectNote||'',photos:data.photos||[],operator_id:u?.userId||'',operator_name:u?.name||'',status:st,recv_time:nowStr()});return{status:st};},
  async review(id,data){const u=getCurrentUser();await db.collection(COL.products).doc(id).update({defect_time:data.defectTime||'',defect_class:data.defectClass||'其他異常',defect_reasons:data.defectReasons||[],defect_note:data.defectNote||'',defect_staff:u?.name||'',status:'procurement'});return{status:'procurement'};},
  async reply(id,data){const u=getCurrentUser();const s=await db.collection(COL.products).doc(id).get();let dt=s.exists?(s.data().defect_time||''):'';if(dt.endsWith('～'))dt+=new Date().toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false});await db.collection(COL.products).doc(id).update({proc_action:data.procAction||'',proc_reply:data.procReply||'',proc_staff_name:u?.name||'',proc_reply_time:nowStr(),defect_time:dt,status:'resolved'});return{status:'resolved'};}
};
